...
module.exports.prototype = {
    ...
    logout: function (auth, cb) {
        this._request('logout', {}, auth, function (err, data) {
            cb && cb(err);
        });
    },
    ...
    queryUser: function (id, auth, cb) {
        this._request('user.query', { id: id }, auth, function (err, data) {
            ...
            var user = _.reduce(fields, function (user, reg, f) {
                var m = reg.exec(body);
                if (m) user[f] = m[1];
                return user;
            }, {});
    ...
    _request: function (api, param, auth, cb) {
        ...
        if (auth) {
            auth.forEach(function (c) { reqParam.jar.add(request.cookie(c)); });
        }
        ...
        request(reqParam, function (e, r, body) {
            ...
            auth = _.map(reqParam.jar.get({ url: 'http://' + me.host }), function (it) { return it.toString(); });
            cb && cb(null, { body: body, auth: auth, statusCode: r.statusCode });
        });
    }
};
